...
# Estructura de contratos esperada, en el orden exacto en el que SIGLE
# debería devolverlos (3 líneas Ordinarias + 3 líneas Discapacidad)
CONTRATOS_ESPERADOS = [
    "Corta duración",
    "Larga duración",
...
def registrar_en_sheets(sheets_service, nombre_gerencia, tipo_lista, contrato, num_gerencia, num_global, fecha_actual):
    if not sheets_service:
        print(
            ...
            "porque sheets_service es None (revisa GOOGLE_CREDENTIALS)."
        )
        return
...
def guardar_debug_html(valor_gerencia, html):
    """Guarda el HTML recibido de SIGLE con timestamp, para no sobrescribir
    capturas anteriores si el fallo se repite varias veces el mismo día."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    try:
        with open(f"debug_{valor_gerencia}_{timestamp}.html", "w", encoding="utf-8") as f:
            f.write(html)
    except Exception as e:
        print(f"Error guardando HTML de depuración para {valor_gerencia}: {e}")
...
def procesar_gerencia(session, sheets_service, nombre, valor_gerencia, thread_id):
    ...
    try:
        ...
        # --- Protección frente a respuestas incompletas o corruptas de SIGLE ---
        if not extraer_view_state(r_final.text):
            print(f"{nombre}: respuesta inválida de SIGLE (sin ViewState).")
            guardar_debug_html(valor_gerencia, r_final.text)
            return
        ...
        if len(filas) != 6:
            print(
                f"{nombre}: respuesta inesperada de SIGLE ({len(filas)} filas). "
                "Se mantiene el estado anterior."
            )
            guardar_debug_html(valor_gerencia, r_final.text)
            return

        contratos = [f.find_all("td")[0].get_text(strip=True) for f in filas]
        if contratos != CONTRATOS_ESPERADOS:
            print(f"{nombre}: estructura inesperada de SIGLE (orden/tipos de contrato no coinciden).")
            guardar_debug_html(valor_gerencia, r_final.text)
            return
        ...
        if datos_actuales != estado_ant:
            print(f"[{nombre}] Cambio detectado, procesando registro en Sheets/Telegram...")
            ahora = datetime.now()
